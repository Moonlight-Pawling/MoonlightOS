...
extern pmode_entry             ; Se define en la sección de 32 bits
...
start:
    ...
    jmp 0x08:pmode_entry       ; Salto a PMODE (selector de código = 0x08)

; ------------------------
; GDT (justo debajo del stub, para que esté <64 KiB)
; ------------------------
gdt_start:
...
global pmode_entry             ; Se define aquí
extern lmode_entry             ; Para salto a 64 bits

pmode_entry:
    ; Cargar todos los selectores de datos con valor 0x10
    mov ax, 0x10
    mov ds, ax
    mov es, ax
    mov fs, ax
    ...
    jmp 0x08:lmode_entry         ; Salto a 64 bits (selector = 0x08)
