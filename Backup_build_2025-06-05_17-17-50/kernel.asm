...
global start                   ; Etiqueta de entrada para el linker
extern pmode_entry             ; Se define en la sección 32 bits
...
start:
    ...
    lgdt [gdt_descriptor]      ; Cargar GDT (definida más abajo)

    mov eax, cr0               ; Activar modo protegido (CR0.PE = 1)
    or eax, 1
    mov cr0, eax

    jmp 0x08:pmode_entry       ; Salto a PMODE (selector código = 0x08)

; ------------------------
; GDT (inmediatamente tras el stub, para que esté <64 KiB)
; ------------------------
gdt_start:
...
extern lmode_entry             ; Saltamos a 64 bits
...
pmode_entry:
    ...
    mov dword [0x3000], 0x0083   ; PD[0] mapea 2 MiB en 0x0 con P=1 | RW=1 | PS=1
    ...
    jmp 0x08:lmode_entry         ; Salto a 64 bits (selector código = 0x08)
...
global lmode_entry              ; Etiqueta de entrada en 64 bits
extern kernel_main              ; Función que implementas en kernel.c

lmode_entry:
    ; Aunque en modo largo los segmentos de datos se ignoran, los cargamos igual
    mov ax, 0x10
    mov ds, ax
    mov es, ax
    mov fs, ax
    ...
    call kernel_main            ; Llamada a la rutina en C
