...
extern start32                  ; Saltamos a start32 en 32 bits
...
start16:
    ...
    jmp 0x08:start32           ; Salto a protected mode (selector = 0x08)
...
global start32                  ; Definimos start32 aquí
extern start64                  ; Saltamos a start64 en 64 bits
...
start32:
    ...
    ; ------------------------
    ; Configurar paginación mínima (2 MiB identity map)
    ; ------------------------
...
global start64                   ; Definimos start64 aquí
extern kernel_main               ; Función C

start64:
    ; En long mode, los segmentos se ignoran, pero cargamos igual
    mov ax, 0x10
    mov ds, ax
    mov es, ax
    mov fs, ax
